...
func (suite *testSuite) getRegion(stepName string, t provider.T, ctx context.Context, api *secapi.RegionV1,
	expectedMeta *schema.GlobalResourceMetadata, expectedSpec *schema.RegionSpec,
) *schema.Region {
	...
	t.WithNewStep(stepName, func(sCtx provider.StepCtx) {
		...
		suite.verifyRegionSpecStep(sCtx, expectedSpec, &resp.Spec)
	})
	return resp

}

func (suite *testSuite) getListRegion(stepName string, t provider.T, ctx context.Context, api *secapi.RegionV1, expectedRegions []schema.Region,
) []*schema.Region {
	var resp []*schema.Region
	...
	t.WithNewStep(stepName, func(sCtx provider.StepCtx) {
		...
		requireNoError(sCtx, err)
		requireNotNilResponse(sCtx, resp)
		requireLenResponse(sCtx, len(resp), len(expectedRegions))
