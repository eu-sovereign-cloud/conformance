...
func (suite *TestSuite) VerifyGlobalResourceMetadataStep(ctx provider.StepCtx, expected *schema.GlobalResourceMetadata, actual *schema.GlobalResourceMetadata) {
	ctx.WithNewStep("Verify metadata", func(stepCtx provider.StepCtx) {
		stepCtx.Require().Equal(expected.Name, actual.Name, "Metadata: Name should match expected")
		stepCtx.Require().Equal(expected.Provider, actual.Provider, "Metadata: Provider should match expected")
		stepCtx.Require().Equal(expected.Resource, actual.Resource, "Metadata: Resource should match expected")
		stepCtx.Require().Equal(expected.ApiVersion, actual.ApiVersion, "Metadata: ApiVersion should match expected")
		stepCtx.Require().Equal(expected.Verb, actual.Verb, "Metadata: Verb should match expected")
		stepCtx.Require().Equal(expected.Kind, actual.Kind, "Metadata: Kind should match expected")
	})
}
...
func (suite *TestSuite) VerifyRegionalResourceMetadataStep(ctx provider.StepCtx, expected *schema.RegionalResourceMetadata, actual *schema.RegionalResourceMetadata) {
	ctx.WithNewStep("Verify metadata", func(stepCtx provider.StepCtx) {
		...
		stepCtx.Require().Equal(expected.Verb, actual.Verb, "Metadata: Verb should match expected")
		stepCtx.Require().Equal(expected.Kind, actual.Kind, "Metadata: Kind should match expected")
		stepCtx.Require().Equal(expected.Tenant, actual.Tenant, "Metadata: Tenant should match expected")
		stepCtx.Require().Equal(expected.Region, actual.Region, "Metadata: Region should match expected")
	})
}
...
func (suite *TestSuite) VerifyRegionalWorkspaceResourceMetadataStep(ctx provider.StepCtx, expected *schema.RegionalWorkspaceResourceMetadata, actual *schema.RegionalWorkspaceResourceMetadata) {
	ctx.WithNewStep("Verify metadata", func(stepCtx provider.StepCtx) {
		...
		stepCtx.Require().Equal(expected.Tenant, actual.Tenant, "Metadata: Tenant should match expected")
		stepCtx.Require().Equal(expected.Workspace, actual.Workspace, "Metadata: Workspace should match expected")
		stepCtx.Require().Equal(expected.Region, actual.Region, "Metadata: Region should match expected")
	})
}
...
func (suite *TestSuite) VerifyRegionalNetworkResourceMetadataStep(ctx provider.StepCtx, expected *schema.RegionalNetworkResourceMetadata, actual *schema.RegionalNetworkResourceMetadata) {
	ctx.WithNewStep("Verify metadata", func(stepCtx provider.StepCtx) {
		...
		stepCtx.Require().Equal(expected.Tenant, actual.Tenant, "Metadata: Tenant should match expected")
		stepCtx.Require().Equal(expected.Workspace, actual.Workspace, "Metadata: Workspace should match expected")
		stepCtx.Require().Equal(expected.Network, actual.Network, "Metadata: Network should match expected")
		stepCtx.Require().Equal(expected.Region, actual.Region, "Metadata: Region should match expected")
	})
}
