...
const (
	...
	StorageSkuName          = "S2XS"
	StorageImageName        = "image1"
	StorageCpuArchitecture  = "x86_64"
	StorageBootVolume       = 20
	StorageSizeGB           = 100
	StorageBlockStorageName = "boot-volume1"
)
...
func TestStorageScenario(t *testing.T) {
	...
	storage := StorageTemplateConfig{
		ImageName:        StorageImageName,
		SkuName:          StorageSkuName,
		BlockStorageName: StorageBlockStorageName,
		CpuArchitecture:  StorageCpuArchitecture,
		BootVolume:       StorageBootVolume,
	...
	err := CreateStorageScenario(MockConfig, storage)
	if err != nil {
		log.Printf("Error creating storage scenario: %v\n", err)
		assert.Error(t, err, "Expected error when creating storage scenario")
	}
